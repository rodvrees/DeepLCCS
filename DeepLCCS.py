...
def get_global_feats(global_arr, df):
    ...
    global_feats = np.concatenate(
        (atom_counts, charge.reshape(-1, 1), normalized_sum_radii.reshape(-1, 1), vols),
        axis=1,
    )
    return global_feats
...
def main():
    ...
    plot_restuls(ccs_df, ccs_df_test, ccs_df_train, args=args)


def plot_restuls(ccs_df, ccs_df_test, ccs_df_train, args={}):
